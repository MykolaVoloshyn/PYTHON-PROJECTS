...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Do you want to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0)
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute(
                """INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",
                ("Nick", "Palmer", "Nick Palmer", "111-111-1111", "palmer@email.com", "history"),
            )
            conn.commit()
    conn.close()
...
def onSelect(self, event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute(
            """SELECT col_fname,col_lname,col_phone,col_email,col_course FROM tbl_students WHERE col_fullname = (?)""",
            [value],
        )
        ...
        for data in varBody:
            ...
            self.txt_info.delete(0, END)
            self.txt_info.insert(0, data[4])
...
def addToList(self):
    ...
    var_info = self.txt_info.get().strip()
    if not "@" or not "." in var_email:  # will use this soon
        print("Incorrect email format!!!")
    if (
        (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0)
        ...
        with conn:
            ...
            cursor.execute(
                """SELECT COUNT(col_fullname) FROM tbl_students WHERE col_fullname = '{}'""".format(var_fullname)
            )  # ,(var_fullname))
            ...
            if chkName == 0:  # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute(
                    """INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",
                    (var_fname, var_lname, var_fullname, var_phone, var_email, var_info),
                )
                ...
                messagebox.showerror(
                    "Name Error",
                    "'{}' already exists in the database! Please choose a different name.".format(var_fullname),
                )
                onClear(self)  # call the function to clear all of the textboxes
        conn.commit()
        conn.close()
